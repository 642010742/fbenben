...
import com.dwz.library.widget.banner.listener.OnBannerListener;
import com.dwz.library.widget.banner.loader.ImageLoaderInterface;
import com.dwz.library.widget.banner.view.BannerViewPager;
...
public class Banner extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    public Banner isShowIndicator(boolean showIndicator) {
    ...
    private void setImageList(List<?> imagesUrl) {
        ...
        initImages();
        for (int i = 0; i <= (isCycle?(count + 1):(count-1)); i++) {
            View imageView = null;
            if (imageLoader != null) {
    ...
    private void setData() {
        ...
        viewPager.setCurrentItem(isCycle?1:0);
        if (gravity != -1)
...
//        Log.i(tag,"currentItem: "+currentItem);
    ...
    public void onPageSelected(int position) {
        ...
        if (mOnPageChangeListener != null) {
            mOnPageChangeListener.onPageSelected(isCycle?toRealPosition(position):position);
        }
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE ||
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE) {
            if (isShowIndicator && isCycle){
    ...
    @Deprecated
    public Banner setOnBannerClickListener(OnBannerListener listener) {
